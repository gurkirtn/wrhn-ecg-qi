...
import { cases, discrepancyData, learningCases, personalTrend, trend12 } from "../lib/mock-data";
import type { Case, Priority } from "../lib/mock-data";
...
const expertNav: NavItem[] = [
  ...
  { href: "/review-queue", label: "Review Queue", icon: ClipboardList, badge: 19 },
  { href: "/cases", label: "ECG Cases", icon: HeartPulse },
      ...
      <div className="main-area">
        ...
        <main className="content">
          <Routes>
            ...
            <Route path="/dashboard" element={role === "expert" ? <ExpertDashboard submissions={reviewSubmissions}/> : <Dashboard submissions={myReviewSubmissions} openUpload={() => setUploadOpen(true)}/>}/>
            ...
            <Route path="/cases/new" element={role === "clinician" ? <CasesPage role={role} submissions={myReviewSubmissions} openUpload={() => setUploadOpen(true)}/> : <Navigate to="/cases" replace/>}/>
            <Route path="/cases/:id" element={visibleCaseSubmission ? <CaseDetail submission={visibleCaseSubmission}/> : <Navigate to="/cases" replace/>}/>
            <Route path="/review-queue" element={role === "expert" ? <ReviewPage submissions={reviewSubmissions} onReviewCompleted={completeExpertSubmission}/> : <Navigate to="/" replace/>}/>
...
function ExpertDashboard({ submissions }: { submissions: ClinicianReviewSubmission[] }) {
  ...
  return <>
    <PageHeader title="Expert Review Overview" subtitle="Cardiology adjudication workspace · High-priority discrepancies first" actions={<Link className="button primary" to="/review-queue"><ClipboardList size={16}/>Open Review Queue</Link>}/>
    ...
    <div className="kpi-grid three">
      <KpiCard icon={ClipboardList} tone="amber" value={String(allAwaiting.length)} label="Awaiting Expert Review" delta={`${submittedAwaiting.length} new`} note="Matches the review queue" to="/review-queue#pending"/>
      <KpiCard icon={AlertTriangle} tone="red" value={String(highPriorityCount)} label="High Priority" delta="Risk sorted" note="Target response: ≤15 min" to="/review-queue#pending"/>
      <KpiCard icon={CheckCircle2} tone="green" value={String(reviewedCount)} label="Expert Reviewed" delta="Completed" note="Matches the review queue" to="/review-queue#completed"/>
    </div>
    <div className="expert-dashboard-grid">
      <Panel title="Priority Review Queue" subtitle="Cases ordered by clinical risk and elapsed time" action={<Link className="text-link" to="/review-queue#pending">View board →</Link>} to="/review-queue#pending">
        <div className="expert-priority-list">{priorityCases.map((item,index)=><Link to="/review-queue#pending" key={item.id}><span className="queue-rank">{index + 1}</span><div><strong className="id-link">{item.patientId}</strong><p>{item.clinicianDx} <span>vs.</span> {item.aiDx}</p></div><PriorityBadge priority={item.priority}/><small><Clock3 size={13}/>{item.elapsed}</small></Link>)}</div>
      </Panel>
      <Panel title="Today’s Workload" subtitle="Same two states as the expert review queue">
        <div className="expert-workload-grid">
          <Link to="/review-queue#pending" className="awaiting"><span><Clock3 size={19}/></span><small>YET TO REVIEW</small><strong>{allAwaiting.length}</strong><p>Severity ordered · {highPriorityCount} high priority</p><b>Open queue →</b></Link>
          <Link to="/review-queue#completed" className="reviewed"><span><CheckCircle2 size={19}/></span><small>EXPERT REVIEWED</small><strong>{reviewedCount}</strong><p>Completed adjudications and feedback</p><b>View completed →</b></Link>
        </div>
